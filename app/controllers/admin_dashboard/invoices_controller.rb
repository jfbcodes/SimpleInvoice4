class AdminDashboard::InvoicesController < AdminDashboard::BaseController
  helper :hot_glue
  ...
  def create
    modified_params = modify_date_inputs_on_params(invoice_params.dup.merge!(user: @user ) )
    ...
    if @invoice.save
      ...
      respond_to do |format|
        format.turbo_stream
        format.html { redirect_to admin_dashboard_user_invoices_path }
      end
    else
  ...
  def update
    ...
    if @invoice.update(modify_date_inputs_on_params(invoice_params))
